...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Check if a specific flavor is passed in the URL
  const flavorFromURL = getQueryParam("flavor");

  if (flavorFromURL) {
    console.log(`Flavor from URL: ${flavorFromURL}`);
    filterProducts(flavorFromURL); // Filter products based on the flavor in URL
  } else {
    displayProducts(products_list); // Display all products if no specific flavor is selected
  }
...
function displayProducts(products) {
  ...
  if (productsSection) {
    productsSection.innerHTML = ""; // Clear the section before adding new products
  }
